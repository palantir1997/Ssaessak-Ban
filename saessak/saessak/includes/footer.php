    ...
    <div id="auth-modal" class="hidden fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 shadow-2xl relative border border-gray-100 transform scale-95 transition-transform duration-300">
            ...
            <div id="modal-login-form" class="space-y-6">
                ...
                <form action="includes/login_process.php" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" id="login-pw" name="password" required placeholder="1234" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-sprout-500 focus:outline-none focus:ring-2 focus:ring-sprout-100 transition-all text-sm" />
                    </div>
            ...
            <div id="modal-signup-form" class="space-y-6 hidden">
                ...
                <form action="includes/signup_process.php" method="POST" class="space-y-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">이름 <span class="text-red-500">*</span></label>
                        <input type="text" id="signup-name" name="name" required placeholder="홍길동" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-sprout-500 focus:outline-none text-sm" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1.5">로그인 ID <span class="text-red-500">*</span></label>
                        <input type="text" id="signup-email" name="login_id" required placeholder="patient02" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-sprout-500 focus:outline-none text-sm" />
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-700 mb-1.5">비밀번호 <span class="text-red-500">*</span></label>
                            <input type="password" id="signup-pw" name="password" required placeholder="8자 이상" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-sprout-500 focus:outline-none text-sm" />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="tel" id="signup-phone" name="phone" required placeholder="010-1234-5678" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-sprout-500 focus:outline-none text-sm" />
                    </div>
                    <button type="submit" class="w-full bg-sprout-600 hover:bg-sprout-700 text-white font-bold py-3.5 rounded-xl shadow-md shadow-sprout-100 transition-all text-sm">
                        회원가입 완료
                    </button>
